...
bool Load3DS(char *filename, ID3D11Device* g_pd3dDevice, ID3D11Buffer **ppVertexBuffer, int *vertex_count)
		...
		switch (l_chunk_id)
			{
				...
				case 0x4110:
					...
					submodels[elem_no].vertices = new PosTexNormVertex[submodels[elem_no].vanz];
	...
	PosTexNormVertex *noIndexVer = new PosTexNormVertex[vertex_anz];
	...
	*vertex_count = vertex_anz;
	...
	bd.ByteWidth = sizeof(PosTexNormVertex) * vertex_anz;
...
bool LoadOBJ(char * filename, ID3D11Device * g_pd3dDevice, ID3D11Buffer ** ppVertexBuffer, int * vertex_count)
	...
	*vertex_count = 3 * faceSize;
	...
	Vertices = new PosTexNormVertex[*vertex_count];
	if (!Vertices)
		{
		return false;
		}
	...
	vertexBufferDesc.ByteWidth = sizeof(PosTexNormVertex) * (*vertex_count);
